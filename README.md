...
Egy [kifejezetten jó leírás található itt a VAE-kről](https://towardsdatascience.com/understanding-variational-autoencoders-vaes-f70510919f73), amely megmutatja a koncepcó mögött meghúzódó Bayesiánus gondolatot is.
...
- A **teljesség** alatt azt értjük, hogy _a tanítóadat mindegyikét_ (kevésbé ideális esetben, "magányos atipikus" train adatok jelenléte mellett: _a tanítóadat zömét_) reprezentálja a latent_dim-beli reprezentáció valamely területe.
...
Ez a loss tag jelenik meg a vaec_trainer.py-ban a calc_loss függvényben a 76-77-ik sorban. Ez a loss tag arra veszi rá a hálónkat, hogy a neck-ben az egymáshoz típusosan hasonlító bemeneteket egymáshoz közeli eloszlásokkal reprezentálja és ezeket minél jobban a normál eloszlás felé próbálja tolni. Ugyanakkor a rekonstrukciós loss tag (vagy később a prediction loss) megpróbálja a különféle típusokat egymástól eltávolítani, hogy azok megkülönböztethetőek legyenek. Ha jól megnézzük a KL divergencia általunk használt közelítését, akkor azt látjuk, hogy a várható érték és a szigma egymástól szeparáltan szerepel az összefüggésben. Vagyis a KL div használatának két hatása van: húzza befelé a (0, 0,...)-hoz a várható értéket, a szigmát pedig húzza 1-hez és ez a kettő nem függ direkten egymástól. A KL divergencia és a rekonstrukciós loss (vagy később a prediction loss) együttesen tehát megvalósítja a teljességet és a folytonosságot is az alábbi módon:
1. Amennyiben a latent_dim-et alacsonyabbra állítjuk, mint a reprezentált típusok száma, a legsűrűbben előforduló train típusokat a neck beli reprezentációban középre (vagyis a (0, 0,...) környékére) húzza, a ritkább esetek a szélekre csúsznak, és a típusokat úgy rendezi el egymás mellett, hogy adott két szomszédos típus között a két típus átmenetét kódolja. Ennek megfelelően azokat a típusokat hajlamos egymás mellé rendezni, amelyeknek köztesei gyakrabban fordulnak elő a tanítóadatban, pl 7-esre hasonlító 1-es, vagy 8-asra hasonlító 3-as, számjegyek esetén.
2. A latent_dim értékét akkor tekinthetjük optimálisnak a teljesség megvalósíthatósága szempontjából, ha latent_dim = $N_{típus}-1$, vagyis latent_dim egyel kisebb a típusok számánál. Ekkor a típusok egyikét véve, az adott típus egyedei egy sokdimenziós tetraéder egyik adott csúcsa körül csoportosulnak. Ilyenkor bármely két, vagy több típus közti átmenet, vagy azok keveréke is magától értetődő módon tud reprezentálódni.
...
- fit_mlp_classifier(): Egybe tanítjuk az encodert és a classifier_head-et kikapcsolt mintavételezéssel. Ekkor a kapott modell tisztán MLP classifiernek tekinthető.
...
- A train adatokkal a latent_dim reprezentációba beszórt **z_mean értékek**re (mintavételezés **ki**kapcsolva, Test0x.ipynb notebookok ábráin a középső és alsó plotok) is ráfittelnék egy többdimenziós Gauss-eloszlást, és arra vonatkozóan, hogy mennyire tekinthetjük az adott inputot a train adatok eloszlásából vett mintavételnek, a Gauss-eloszlás sűrűségfüggvényéből képeznék konfidencia értéket: $conf = \dfrac{\varphi_{\mu, \sigma}(z_{mean})}{\varphi_{\mu, \sigma}(\mu)}$.
