...
- A **folytonosság** pedig azt jelenti, hogy a latent_dim-beli reprezentáció minden lehetséges pontja - egy központi területen - "értelmes" kimenő adatot reprezentál, az adott pontot a Decoder-re beadva éretlmezhető rekonstrukciót kapunk - pl számjegyet, ha számjegyekkkel tanítottunk, vagy emberi arcokat, ha emberi arcokkal tanítottunk, ráadásul olyat, ami a tanítóadatból hiányzik, de mégsem "lóg ki" belőle. Ezt a reconstrukciós loss (pl L2(input-output)) és a KL divergencia együttes használatával érjük el.
...
2. a típusokat úgy rendezi el egymás mellett, hogy adott két szomszédos típus között a két típus átmenetét kódolja. Ennek megfelelően azokat a típusokat hajlamos egymás mellé rendezni, amelyeknek köztesei gyakrabban fordul elő a tanítóadatban, pl 7-esre hasonlító 1-es, vagy 8-asra hasonlító 3-as.
...
-  A test0x.ipynb-ok demonstrálnak pár tanítást latent_dim=2-nél, különösen a test03.ipynb érdekes. A notebookokban ábrákat készítettem 8, 16, 32, 64, 128, 256, 512, 1000, 2000, ... 10000 epoch után. Az ábrák bal oldalán mindig a train adattal, a jobb oldalon teszt adattal készített plotok vannak. Felül a ténylegesen mintavételzett eloszlások láthatók fixen [-5, 5] tengelymérettel. Középen a z_mean-ok láthatóak fixen [-3, 3] tengelymérettel, alul ugyanez, de "rugalmas" [min, max] tengelyméretekkel. Ezek az ábrák szépen megmutatják, hogy a tanítás során hogyan szeparálódnak a 0 és 1 labelű classok, illetve, hosszú tanítás után a reprezentáció végül 1 dimenzióba "szorul", és a tanítási adat reprezentációja a két osztályra markánsan elkülönül, lásd a legutolsó ábrát.
