...
class VAEC_Trainer(tf.keras.Model):
    def __init__(self, encoder, decoder, classifier_head, train_what, **kwargs):
        ...
        *Variational AutoEncoder + Classifier Trainer*
        ...
        - 'mlp_classifier': Only encoder and classifier_head is being trained.
        """
        ...
        if self.train_what == 'all':
            ...
            raise TypeError(
                "Parameter train_what must be 'all', 'autoencoder', 'classifier_head' or 'mlp_classifier'!")
    ...
    def calc_loss(self, x, y):
        ...
        if self.train_what == 'all':
            total_loss = kl_loss + recon_loss + pred_loss
        elif self.train_what == 'autoencoder':
            total_loss = kl_loss + recon_loss
        elif self.train_what == 'classifier_head' or self.train_what == 'mlp_classifier':
            total_loss = pred_loss
        else:
            raise TypeError("Something is not OK, train_what was checked in __init__.")

        return total_loss, recon_loss, kl_loss, pred_loss, pred
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            x = data[0]
            y = data[1]
            total_loss, recon_loss, kl_loss, pred_loss, pred = self.calc_loss(x, y)
